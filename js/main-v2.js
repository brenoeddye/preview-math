...
(function () {
  ...
  /* ----------------------------------------------------------
     REVEAL v2 - Cards de módulos e target audience
     ---------------------------------------------------------- */
  const v2RevealElements = document.querySelectorAll('.module-card-v2, .target-card, .stat-card');

  if ('IntersectionObserver' in window) {
    const v2RevealObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
    ...
    v2RevealElements.forEach(function (el, index) {
      el.style.opacity = '0';
      el.style.transform = 'translateY(30px)';
      el.style.transition = 'opacity 0.6s ease ' + (index * 0.05) + 's, transform 0.6s ease ' + (index * 0.05) + 's';
      v2RevealObserver.observe(el);
    });
  }

  // FAQ accordion é tratado pelo script inline do index2.html
